...
circuit or network or topology (used interchangeably) and analyze the generated data. 
...
rSet <- sRACIPE::simulateGRC(circuit = demoCircuit, numModels = 500,
                             plots = FALSE)
...
In practice, we found nNoise ~ 30 and initialNoise ~50 divided by sqrt(number_gene) 
...
```{r eval=TRUE}
rSet <- sRACIPE::simulateGRC(circuit = demoCircuit, numModels = 100, plots = FALSE)
params <- params(rSet)
